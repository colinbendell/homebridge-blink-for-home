...
class BlinkAPI {
  constructor(clientUUID, auth = { path: "~/.blink", section: "default" }) {
    let ini = readini(
      process.env.BLINK || auth.path,
      process.env.BLINK_SECTION || auth.section
    );
  ...
  async _request(
    ...
    if (CACHE.has(method + targetPath) && maxTTL > 0) {
      ...
      let lastModified = Date.parse(
        cache.headers.get("last-modified") || cache.headers.get("date") || 0
      );
      if (lastModified + maxTTL * 1000 > Date.now()) {
        ...
        cache.headers.set(
          "last-modified",
          new Date(Date.now() + 3 * 1000).toISOString()
        );
      }
    }
    ...
    let res = await fetch(`${urlPrefix}${targetPath}`, options).catch((e) => {
      this.log.error(e);
    ...
    if (res.status === 401) {
      ...
      this.log.error(
        `${method} ${targetPath} (${
          res.headers.get("status") || res.status + " " + res.statusText
        })`
      );
  ...
  async login(
    ...
    if (!res || /unauthorized|invalid/i.test(res.message)) {
      ...
      const data = Object.assign(
        { authtoken: {}, account: {}, client: {}, region: {} },
        res
      );
      ...
      if (res.client.verification_required && this.auth.pin)
        await this.verify();
    }
  ...
  async getClientOptions() {
    return await this.get(
      `/api/v1/accounts/{accountID}/clients/{clientID}/options`
    );
  }
  ...
  async deleteMedia(medialist = []) {
    ...
    return await this.post(`/api/v1/accounts/{accountID}/media/delete`, {
  ...
  async updateCommand(networkID, commandID, updateCommandRequest) {
    return await this.post(
      `/network/${networkID}/command/${commandID}/update/`,
  ...
  async createProgram(networkID, program) {
    return await this.post(
      `/api/v1/networks/${networkID}/programs/create`,
      program
    );
  }
  ...
  async updateProgram(networkID, programID, updateProgramRequest) {
    return await this.post(
      `/api/v1/networks/${networkID}/programs/${programID}/update`,
  ...
  async deleteProgram(networkID, prgoramID) {
    return await this.post(
      `/api/v1/networks/${networkID}/programs/${prgoramID}/delete`
    );
  }
  ...
  async getCameraMotionRegions(networkID, cameraID) {
    return await this.get(
      `/api/v1/accounts/{accountID}/networks/${networkID}/cameras/${cameraID}/motion_regions`
    );
  }
  ...
  async updateCameraTemperature(
    ...
    return await this.post(
      `/api/v1/network/${networkID}/camera/${cameraID}/calibrate`,
      body
    );
  }
  ...
  async disableCameraTempAlert(networkID, cameraID) {
    return await this.post(
      `/api/v1/network/${networkID}/camera/${cameraID}/temp_alert_disable`
    );
  }

  async enableCameraTempAlert(networkID, cameraID) {
    return await this.post(
      `/api/v1/network/${networkID}/camera/${cameraID}/temp_alert_enable`
    );
  }
  ...
  async activateSiren(networkID, sirenID, duration = 30) {
    return await this.post(
      `/api/v1/networks/${networkID}/sirens/${sirenID}/activate/`,
      { duration }
    );
  }

  async activateSirens(networkID, duration = 30) {
    return await this.post(`/api/v1/networks/${networkID}/sirens/activate/`, {
      duration,
    });
  }
  ...
  async createSiren(networkID, addSirenNetworkBody) {
    return await this.post(
      `/api/v1/networks/${networkID}/sirens/add/`,
      addSirenNetworkBody
    );
  }
  ...
  async deleteSirens(networkID, sirenID) {
    return await this.post(
      `/api/v1/networks/${networkID}/sirens/${sirenID}/delete`
    );
  }

  async updateSiren(networkID, sirenID, sirenNameBody) {
    return await this.post(
      `/api/v1/networks/${networkID}/sirens/${sirenID}/update`,
      sirenNameBody
    );
  }
  ...
  async updateSirens(networkID, sirenDurationBody) {
    return await this.post(
      `/api/v1/networks/${networkID}/sirens/update`,
      sirenDurationBody
    );
  }
  ...
  async createChime(networkID, addSirenNetworkBody) {
    return await this.post(
      `/api/v1/accounts/{accountID}/networks/${networkID}/chimes/add/`,
      addSirenNetworkBody
    );
  }
  ...
  async createOwl(networkID, addOwlBody) {
    return await this.post(
      `/api/v1/accounts/{accountID}/networks/${networkID}/owls/add`,
      addOwlBody
    );
  }
  ...
  async deleteSyncModule(networkID, syncModuleID) {
    return await this.post(
      `/network/${networkID}/syncmodule/${syncModuleID}/delete/`
    );
  }
